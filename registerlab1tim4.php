...
if(isset($_POST["submit"]))
{
    ...
    $connect=mysqli_connect($hostname,$username,$password,$databaseName);
     ...
     $ative = mysqli_query($connect,"SELECT active5 FROM country_state_city_save WHERE `Day`='".$id."'");
     $result2 = mysqli_fetch_array($ative);
       
         if($result2['active5']==0){
           //echo("You have successfuly ");
         //$id=$_POST['id'];
         //$lab=$_POST['country'];
         //$subject=$_POST['state'];

     $query="UPDATE `country_state_city_save` SET `active5`=1, `country`=$lab,`4:30-6:30`='".$subject."' WHERE `Day`='".$id."'and `active5`=0";
...
<form  name="myForm"  method="POST">
        ...
        <option  value="registerlab1tim1.php">8:30-10:30</option>
        <option value="registerlab1tim2.php">11:00-1:00</option>
        <option value="registerlab1tim3.php">2:00-4:00</option>
        <option selected="selected">4:30-6:30</option>
        <option value="registerlab1tim5.php">6:30-8:30</option>
      </select>
      <br>
