...
if(isset($_POST["submit"]))
{
    ...
    $connect=mysqli_connect($hostname,$username,$password,$databaseName);
     ...
     $ative = mysqli_query($connect,"SELECT active3 FROM country_state_city_save WHERE `Day`='".$id."'");
     $result2 = mysqli_fetch_array($ative);
       
         if($result2['active3']==0){
           //echo("You have successfuly ");
         //$id=$_POST['id'];
         //$lab=$_POST['country'];
         //$subject=$_POST['state'];

     $query="UPDATE `country_state_city_save` SET `active3`=1, `country`=$lab,`3rd`='".$subject."' WHERE `Day`='".$id."'and `active3`=0";
...
<form  name="myForm"  method="POST">
        ...
        <option selected="selected">2:00-4:00</option>
        <option value="registerlab1tim5.php">4:30-6:30</option>
        <option value="registerlab1tim6.php">6:30-8:30</option>
      </select>
      <br>
