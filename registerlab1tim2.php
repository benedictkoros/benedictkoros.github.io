...
if(isset($_POST["submit"]))
{
    ...
    $connect=mysqli_connect($hostname,$username,$password,$databaseName);
     ...
     $ative = mysqli_query($connect,"SELECT active2 FROM country_state_city_save WHERE `Day`='".$id."'");
     $result2 = mysqli_fetch_array($ative);
       
         if($result2['active2']==0){
           //echo("You have successfuly ");
         //$id=$_POST['id'];
         //$lab=$_POST['country'];
         //$subject=$_POST['state'];

     $query="UPDATE `country_state_city_save` SET `active2`=1, `country`=$lab,`state2`='".$subject."' WHERE `Day`='".$id."'and `active2`=0";
...
<form  name="myForm"  method="POST">
        ...
        <option selected="selected">11:00-1:00</option>
