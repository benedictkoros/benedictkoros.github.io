...
if(isset($_POST["submit"]))
{
    ...
    $connect=mysqli_connect($hostname,$username,$password,$databaseName);
     ...
     $ative = mysqli_query($connect,"SELECT active6 FROM country_state_city_save WHERE `Day`='".$id."'");
     $result2 = mysqli_fetch_array($ative);
       
         if($result2['active6']==0){
           //echo("You have successfuly ");
         //$id=$_POST['id'];
         //$lab=$_POST['country'];
         //$subject=$_POST['state'];

     $query="UPDATE `country_state_city_save` SET `active6`=1, `country`=$lab,`6:30-8:30`='".$subject."' WHERE `Day`='".$id."'and `active6`=0";
...
<form  name="myForm"  method="POST">
        ...
        <option value="registerlab1tim4.php">4:30-6:30</option>
        <option selected="selected">6:30-8:30</option>
      </select>
      <br>
